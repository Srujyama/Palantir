...
titlebar, or `Shift+S`): it walks queue → P-1002 → trajectory → sandbox →
capacity → model card → analytics → floor with a caption per beat, so you
can narrate over it. The shot list below is the manual click-path; the
beat order matches StoryMode's.
...
| 1:42–2:18 | `/capacity` (Capacity / What-If). Baseline census step chart + "Beds free now" KPI. In the **Scenario builder**, toggle the `dispo delay` chip (count shows 32), click **Run scenario**. The **Scenario impact** card animates: read the big "+N beds by 24h" number off the screen; scroll to the **Freed patients** table; flash open **Model assumptions**. | "Surfacing problems is half the job — this page turns it into a decision. Baseline: forty-eight hours of projected free beds, from a deterministic length-of-stay model. Now the what-if: resolve every discharge-and-placement hold, and run it. [read the screen] Plus thirty-two beds within a day — and here are the thirty-two named patients whose discharge actually moves. No ML, no black box: every number traces to an assumption stated right here on the page." |
