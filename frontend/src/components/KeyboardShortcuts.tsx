...
export function KeyboardShortcuts() {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (pendingG) {
        ...
        else if (k === "y") navigate("/capacity");
        else if (k === "s") navigate("/sandbox");
      ...
      if (e.key === "g") {
        setPendingG(true);
        // Reset any in-flight chord timer so a fresh g always gets the full
        // window — a stale timeout must not cancel the newer chord.
        if (gTimer.current !== null) window.clearTimeout(gTimer.current);
        gTimer.current = window.setTimeout(() => setPendingG(false), 1500);
      }
    };
  ...
  return (
    <>
      {pendingG && (
        <div className="kb-hint mono">g · waiting · q queue · f floor · a analytics · h handoff · y capacity · s sandbox · l landing</div>
      )}
      {showHelp && (
          ...
          <div className="kb-help" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="kb-help-grid">
              <div className="kb-help-section">
                ...
                <div className="kb-row"><kbd>g</kbd><kbd>y</kbd><span>Capacity</span></div>
                <div className="kb-row"><kbd>g</kbd><kbd>s</kbd><span>Sandbox</span></div>
